...
for err in range(8,25):
    ...
    fin.readinto(delta)
    fin.close()
    zfpcmpresscmd = '/home/luo/ZFP/zfp/bin/zfp -a '+str(PW_E)+' -s -d -1 '+str(cnt[3])+' -i Level_3.dat -z Level_3.dat.zfp'
    zfpdecmp = '/home/luo/ZFP/zfp/bin/zfp -a  '+str(PW_E)+' -d -1 '+str(cnt[3])+ ' -z Level_3.dat.zfp -o Level_3.dat.zfp.out'
    print zfpdecmp
    os.system(zfpcmpresscmd)

    os.system(zfpdecmp)
    fin = open('Level_3.dat.zfp.out', 'rb')
