...
rects1 = ax.bar(ind-width, y1, width, color='g',label="Baseline+ZFP")
rects2 = ax.bar(ind, y2, width, color='r',label="LevelRe+ZFP")
rects3 = ax.bar(ind+width, y3, width, color='b',label="Baseline+SZ")
rects4 = ax.bar(ind+2*width, y4, width, color='y',label="LevelRe+SZ")
...
#ax.set_yscale('log')
ax.set_ylim([0,1.4])
#ax.set_title("MaxError comparison",axis_font)
ax.set_xticks(ind+width)
ax.set_xticklabels( namelist )
ax.set_xlim([-0.5,18])
