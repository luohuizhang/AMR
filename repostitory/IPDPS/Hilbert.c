...
void leveldata_box_zordering(struct datapoint **data, int cnt [Level],struct box **boxes,int box_cnt[Level],int **processors,int col)
{
	char filename[50];
...
void leveldata_box_zordering_level(struct datapoint **data, int cnt [Level],struct box **boxes,int box_cnt[Level],int** processors,int col)
{
	char filename[50];
...
int main()
{
	...
	for(int col=0;col<6;col++){
		...
		leveldata_box_zordering(data,cnt,boxes,box_cnt,processors,col);
		leveldata_box_zordering_level(data,cnt,boxes,box_cnt,processors,col);
		for(i=0;i<Level;i++)	
			free(data[i]);
		for(i=0;i<Level;i++)
			free(boxes[i]);
