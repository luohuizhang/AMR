...
size=$(( $size+ $(stat -c%s $i)/8 ))
	...
	echo $(stat -c%s $i)/$(stat -c%s $i.zfp) |bc -l>>../Large/result_largerdataset.txt 
...
#/home/luo/ZFP/SZ/bin/testdouble_compress /home/luo/ZFP/SZ/sz.config  $i $size
	...
	echo $(stat -c%s $i)/$(stat -c%s $i.sz) |bc -l>>../Large/result_largerdataset.txt
