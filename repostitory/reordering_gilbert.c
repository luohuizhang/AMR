...
void leveldata_box_gilbert(struct datapoint **data, int cnt [Level],struct box **boxes,int box_cnt[Level],int col)
{
	char filename[50];
	sprintf(filename,"Level_box_gilbert_%d.dat",col);
...
void leveldata_box_gilbert_level(struct datapoint **data, int cnt [Level],struct box **boxes,int box_cnt[Level],int col)
{
	char filename[50];
	sprintf(filename,"Level_box_gilbert_levelreordering_%d.dat",col);
	FILE *fp=fopen(filename,"w");
...
int main()
{
	...
	for(int col=0;col<18;col++){
		...
		leveldata_box_gilbert(data,cnt,boxes,box_cnt,col);
		leveldata_box_gilbert_level(data,cnt,boxes,box_cnt,col);
		for(i=0;i<Level;i++)	
			free(data[i]);
		for(i=0;i<Level;i++)
			free(boxes[i]);
